...
public class FuelSumService {
    ...
    private static MultiValuedMap<Double,Double> getCount(final String str) {
        ...
        final Pattern p2 = Pattern.compile("\\b[.?]LEJ [.?]APĘDOWY\\S*\\(?\\d?\\)?\\s*([\\d+]{1,5}\\.?[\\d]{0,5})\\b|\\bVERVA 95\\S*\\(?\\d?\\)?\\s*([\\d+]{1,5}\\.?[\\d]{0,5})\\b|\\bVERVA 98\\S*\\(?\\d?\\)?\\s*\\b\\s*([\\d+]{1,5}\\.?[\\d]{0,5})|\\bGAZ LPG\\S*\\(?\\d?\\)?\\s*\\b\\s*([\\d+]{1,5}\\.?[\\d]{0,5})"); // w tym przypadku dodamy wiecej nazw
        ...
        while (matcher.find() && matcher2.find()) {
            countValues.put(Double.parseDouble(matcher2.group().replaceAll("\\b[.?]LEJ [.?]APĘDOWY\\S*\\(?\\d?\\)?\\s*\\b|\\bVERVA 98\\S*\\(?\\d?\\)?\\s*\\b|\\bGAZ LPG\\S*\\(?\\d?\\)?\\s*\\b\\b|\\bVERVA 95\\S*\\(?\\d?\\)?\\s*\\b","")),Double.parseDouble(matcher.group(1)));// w tym przypadku dodamy wiecej nazw
